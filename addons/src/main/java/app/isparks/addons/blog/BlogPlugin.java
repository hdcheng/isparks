 ...
 * @date： 2021/3/19
 */
public class BlogPlugin implements IPlugin {

    @Override
    public void activate(Object var) {
        // 初始化配置
        ISparksApplication.instance().getPluginManager().ifPresent( pm -> run(pm) );
        // 初始化数据库
        IOCUtils.getBeanByClass(ISysService.class).ifPresent(sysService -> initDB(sysService));
    }
    ...
    private void run(PluginManager pm){
...
//        pm.setLinkButton("blog","博客管理",svg);

//        // 注册mapper
//        PostAttachMapper mapper = pm.registerMyBatisMapper("postAttachMapper",PostAttachMapper.class).orElseThrow(()->new PluginException("注册 mapper 失败"));
//
